...
- Make a folder inside your `homework` folder for w02d05 called `hogwarts`.
...
- [Open up your jQuery magic markdown file](https://git.generalassemb.ly/Software-Engineering-Immersive-Remote/SEIR-Devito/wiki/JQuery-Cheatsheet-as-Tables)- for reference
...
A `hogwarts` folder with `index.html`, `main.css`, and `app.js` files that do all the above from Year 1 to 7

## Technical Requirements
1. Your file must run on first load with no syntax errors. If there are errors you can't solve, comment them out and leave a comment above it explaining what is wrong
